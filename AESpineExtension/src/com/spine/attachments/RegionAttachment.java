...
public class RegionAttachment extends Attachment	
{
	...
	private SpineTextureRegion region;
	...
	public void updateOffset () {
		...
		if (region instanceof SpineTextureRegion) {
			SpineTextureRegion region = (SpineTextureRegion)this.region;
			 if (region.isRotated()) {
		...
		float dX = getX();
		float dY = getY();
		float localXCos = localX * cos + dX;
		float localXSin = localX * sin;
		float localYCos = localY * cos + dY;
		float localYSin = localY * sin;
		float localX2Cos = localX2 * cos + dX;
		float localX2Sin = localX2 * sin;
		float localY2Cos = localY2 * cos + dY;
	...
	public void setRegion (SpineTextureRegion pRegion) {
		if (pRegion == null) throw new IllegalArgumentException("region cannot be null.");
		this.region = pRegion;
		float[] vertices = this.vertices;	
		if (pRegion instanceof SpineTextureRegion && ((SpineTextureRegion)pRegion).isRotated()) {
			vertices[U2] = pRegion.getU();
			vertices[V2] = pRegion.getV2();
			vertices[U3] = pRegion.getU();
			vertices[V3] = pRegion.getV();
			vertices[U4] = pRegion.getU2();
			vertices[V4] = pRegion.getV();
			vertices[U1] = pRegion.getU2();
			vertices[V1] = pRegion.getV2();
		} else {
			vertices[U1] = pRegion.getU();
			vertices[V1] = pRegion.getV2();
			vertices[U2] = pRegion.getU();
			vertices[V2] = pRegion.getV();
	...
	public SpineTextureRegion getRegion () {
